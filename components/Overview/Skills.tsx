        ...
        {SKILLS_TITLES.map((title, index) => {
          ...
          return (
            <button
              ...
              className={`rounded-full cursor-pointer text-white px-5 py-3 bg-buttonInital ${
                isActive
        ...
        <table className="table-fixed w-full border-tableBorder border-2 text-white">
          ...
          <tbody>
            {currentSkill.map((skill, index) => (
              <tr key={index}>
                <td className="w-1/4 px-4 py-2 text-center border-tableBorder border-2 bg-tableBg text-tableText">
                  {skill.title}
                </td>
